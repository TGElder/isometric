...
impl ResizeRelay{
    ...
    fn get_physical_size(&self, logical_size: glutin::dpi::LogicalSize) -> glutin::dpi::PhysicalSize {
...
impl EventHandler for ResizeRelay {
    fn handle_event(&mut self, event: Arc<Event>) -> Vec<Command> {
        match *event {
            ...
            ) => vec![Command::Event(Event::Resize(self.get_physical_size(logical_size)))],
